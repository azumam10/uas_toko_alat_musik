...
void create_user(const string& nama, int harga, const string& kategori, const string& merek) {
    MYSQL* conn = connect_db();
    if (conn) {
        stringstream query;
        query << "INSERT INTO alat_musik (nama, kategori, merek, harga) VALUES ('" << nama << "', '" << kategori << "', '" << merek << "', " << harga << ")";
...
void get_users() {
    MYSQL* conn = connect_db();
    if (conn) {
        if (mysql_query(conn, "SELECT * FROM alat_musik")) {
        ...
        while ((row = mysql_fetch_row(res))) {
            cout << "ID: " << row[0] << ", Nama: " << row[1] << ", Kategori: " << row[2] << ", Merek: " << row[3] << ", Harga: " << row[4] << endl;
        }
...
void update_user(int user_id, const string& nama, const string& kategori, const string& merek, int harga) {
    MYSQL* conn = connect_db();
    if (conn) {
        stringstream query;
        query << "UPDATE alat_musik SET nama = '" << nama << "', kategori = '" << kategori << "', merek = '" << merek << "', harga = " << harga << " WHERE id = " << user_id;
        if (mysql_query(conn, query.str().c_str())) {
...
void delete_user(int user_id) {
    ...
    if (conn) {
        stringstream query;
        query << "DELETE FROM alat_musik WHERE id = " << user_id;
        if (mysql_query(conn, query.str().c_str())) {
...
void tampilan() {
    ...
    while (true) {
        cout << "====================================\n";
        cout << RED << "Toko Fals Scream Radio Berkah Jaya\n" << RESET;
        cout << "====================================\n";
        cout << "\nMenu:\n";
        ...
        if (choice == 1) {
            if (is_admin) {
                ...
                create_user(nama, harga, kategori, merek);
            } else {
            ...
            if (is_admin) {
                ...
                update_user(user_id, nama, kategori, merek, harga);
            } else {
                cout << "Permission denied. You can only view data." << endl;
            }
        } else if (choice == 4) {
...
// cd uas_curd (untuk masuk folder)
// docker compose up -d --build (membuat docker)
// ./crud_simple (menjalankan program)
